...
void IMU::readData() {
  ...
  /*
    ...
    for (int i = 0; i < 3; i++)
    {
      strData += string(accelData[i]);
      strData += ",";
      strData += string(gyroData[i]);
      strData += ",";
      strData += string(magData[i]);
      strData += ",";
    }
  */ 
    ...
    stream << "Accel: " << DMPdata.Raw_Accel.Data.X << ", " << DMPdata.Raw_Accel.Data.Y << ", " << DMPdata.Raw_Accel.Data.Z << "; ";
    stream << "Gyro: " << DMPdata.Raw_Gyro.Data.X << ", " << DMPdata.Raw_Gyro.Data.Y << ", " << DMPdata.Raw_Gyro.Data.Z << "; ";
    stream << "Mag: " << DMPdata.Compass.Data.X << ", " << DMPdata.Compass.Data.Y << ", " << DMPdata.Compass.Data.Z;
    dataString = stream.str();

    // Imprimindo a string resultante
    std::cout << dataString << std::endl;

    // writeFile("/teste_file.csv", dataString);
  }
