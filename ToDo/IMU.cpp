...
bool IMU::init() {
	...
	while (!initialized) {
		...
		if (myICM.status != ICM_20948_Stat_Ok) {
			LogWarningln("Retry initialization...");
			delay(500);
		} else {
			LogInfoln("Sensor connected successfully.");
			initialized = true;
		}
		...
		if (success) {
			LogInfoln("DMP enabled and configured successfully.");
		} else {
			LogErrorln("DMPenabled and configuration failed.");
			LogErrorln("Ensure line 29 (`#define ICM_20948_USE_DMP`) in `ICM_20948_C.h` is uncommented...");
			while (1)
				; // Halt on failure
		}
	}
...
void IMU::printData() {
	...
	LogVerbose("IMU ID: ");
	LogVerbose(IMUnumber);
	LogVerbose(" | Accel (X, Y, Z): ");
	LogVerbose(accelData[0]);
	LogVerbose(", ");
	LogVerbose(accelData[1]);
	LogVerbose(", ");
	LogVerbose(accelData[2]);
	LogVerbose(" | Gyro (X, Y, Z): ");
	LogVerbose(gyroData[0]);
	LogVerbose(", ");
	LogVerbose(gyroData[1]);
	LogVerbose(", ");
	LogVerbose(gyroData[2]);
	LogVerbose(" | Mag (X, Y, Z): ");
	LogVerbose(magData[0]);
	LogVerbose(", ");
	LogVerbose(magData[1]);
	LogVerbose(", ");
	LogVerboseln(magData[2]);
}
